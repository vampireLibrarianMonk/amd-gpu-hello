...
# From https://github.com/johnnynunez/rocm_lab/blob/master/scripts/build_tensorflow.sh
# Edited to work with amd-gpu-hello
...
sudo apt-get update && sudo apt-get install -y curl openjdk-8-jdk openjdk-8-jre python${PYTHON_VERSION}-venv python3-pip unzip wget git libstdc++-12-dev
python3 -m venv venv
...
export PYTHON_LIB_PATH=$WORKDIR/venv/lib/python${PYTHON_VERSION}/site-packages
